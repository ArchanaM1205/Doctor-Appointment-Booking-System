  ...
  <div class="modal">
    ...
    <form id="loginForm">
      <div class="form-group">
        ...
        <input type="email" id="email" placeholder="Enter your username" name="email" required />
      </div>
      ...
      <div class="forgot-password">
        <a href="forgot-password-doctor.py">Forgot Password?</a>
      </div>

      <input type="submit" class="submit-btn" name="sub" value="Login">
      <button type="button" class="cancel-btn" id="cancelBtn">Cancel</button>
    </form>

    <div class="login-footer">
      Don't have an account? <a href="register-doctor.py">Sign Up</a>
    </div>
  </div>
</div>
...
if submit != None:
    ...
    q = """select id from doctor_register where Email='%s' and Password='%s'""" % (u_name, password)
    cur.execute(q)
    det = cur.fetchone()
    if det != None:
        print("""
        <script>
        alert("Login success");
        window.location.href="doctor-dashboard.py?id=%s";
        </script>
        """ % det[0])
    else:
        print("""
